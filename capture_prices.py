...
# ── Tickers — must match your notebook ────────────────────────────────────────
TICKERS = [
...
def is_market_holiday():
    """
    Very lightweight holiday check — just skips obvious US market holidays.
...
def main():
    ...
    # ── Skip weekends (Actions cron is weekday-only but belt-and-suspenders) ──
    ...
    # yfinance returns a MultiIndex when group_by='ticker'
    # Flatten to a simple Close-only DataFrame
    try:
        close = raw.xs("Close", axis=1, level=1)
    except KeyError:
        # Single ticker fallback (shouldn't happen with 10 tickers)
        close = raw["Close"]
